...
It turns out that some of MT genes were not included in the original calculation of ```% MT transcrips per cell```.  I created ```WF26CD34.su``` obj that only contains the analysis of the subject, ```WF26``` with ```CD34``` cells by spliting the integrated Seurat obj 
