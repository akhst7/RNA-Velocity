...
In python, bioinformatics data including scRNAseq ones are generally handeled  by an ```Anndata``` package.  The ```Anndata``` package creates a data objects that resembles the one from ```SingleCellExperiment```.  
